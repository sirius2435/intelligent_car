...
确认 END 后仍切换为 1/4（120x80）解码，保留球和洞口框作为固定路线的
标定辅助；推球控制本身不依赖这些检测结果。UVC 分辨率、输入帧率和专用
解码任务均不改变，因此不会降低前半程巡线的识别帧率。
...
球应放在各自洞口向场内延伸的中心线上，并用胶带或定位模板固定球心位置。
固定路线没有视觉纠偏，因此只有摄像头确认 END 横条后才允许启动；普通丢线
会锁定停车。

动作顺序为：

1. 停稳后定距前进到公共起始点。
2. 编码器定角向右旋转，使车头朝向洞口所在边缘。
3. 横移到红球线路，低速直推红球。
4. 后退相同计数并反向横移，回到公共横向基准。
5. 横移到白球线路，低速直推白球，然后停车完成。

两个洞口位于同一边缘，两条入洞延长线平行，所以第二球不再重复旋转。
每段动作之间停车 `BALL_SCRIPT_SETTLE_MS`；直行和旋转要求左右轮中的较慢轮
达到目标，横移要求归一化后的三轮均达到目标。单轮空转不会提前结束，堵转
或阶段超时会进入 `FAULT_STOP`。

所有现场参数集中在 `main/board_config.h` 的 `BALL_SCRIPT_*`：

- `ADVANCE_COUNTS`：END 到公共起始点的距离。
- `ROTATE_RIGHT_TURN`：复用巡线原地转向输出 `LINE_CORNER_ROTATE_TURN`。
- `ROTATE_RIGHT_COUNTS`：由巡线 30° 编码器标定推导，当前值 240 是约 90°
  的初始估计。
- `RED_ALIGN_*`、`WHITE_ALIGN_*`：从公共基准到两条球道的横移方向和距离。
- `RED_PUSH_COUNTS`、`WHITE_PUSH_COUNTS`：把球持续推过洞口所需距离。

首次标定应架空确认动作方向，再把后续阶段计数临时设为 0，一次只测一个
阶段。优先标定右转，其次两条横移，最后从低值逐步增加推球距离。视觉检测
和 Wi-Fi 框仍保留作观察工具，但不参与控制判定。
