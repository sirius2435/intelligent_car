    ...
    while (1) {
        ...
        if (mission == MISSION_LINE && !locked) {
            ...
            if (control.state == LINE_FOLLOW_STOPPED) {
                ...
                if (!sensor.finish_detected) {
                    ...
                    /* From here on the ball detections drive the car: the shot
                     * line is found by strafing and rotating on the decoded
                     * ball frame instead of by a scripted turn. */
            ...
            ball_control = ball_push_update(
                &ball_controller, &camera.ball, elapsed_ms, left_count,
                right_count, rear_count);
            forward = ball_control.forward;
            lateral = ball_control.lateral;
            turn = ball_control.turn;
            if (ball_control.just_completed_ball) {
                ESP_LOGI(TAG, "%s push finished: %s",
                         ball_control.target_color == BALL_COLOR_RED ?
                             "red" : "white",
                         ball_control.ball_scored ?
                             "ball left the frame" : "encoder budget reached");
            }
            if (ball_control.fault) {
                ESP_LOGE(TAG, "ball task failed in %s (ball_err=%d hole_err=%d)",
                         ball_push_state_name(ball_control.state),
                         ball_control.ball_error, ball_control.hole_error);
                locked = true;
        ...
        if (log_elapsed_ms >= LINE_LOG_PERIOD_MS) {
            ...
            ESP_LOGI(TAG,
                     ...
                     "det=[R:%d W:%d HL:%d HR:%d L:%d] shot=[%d,%d] "
                     ...
                     camera.ball.lone_hole.found,
                     ball_controller.ball_error,
                     ball_controller.hole_error,
