...
不接串口、不装任何 App：车自己发一个 WiFi 热点，手机或电脑连上后用浏览器看摄像头原始画面 + 算法实时判定叠加。
...
| 区域 | 说明 |
|---|---|
| 大图 | 摄像头原始 MJPEG 画面（480x854 竖排，约 5 fps，跟随视觉解码速度） |
...
调视觉算法时盯"大图 + 误差条"就能判断：画面里线明明在中间但误差不为 0 → 分割/ROI 参数问题；`valid_rows` 少 → 阈值 `VISION_LINE_CONFIDENCE_MIN` 太严等。
...
| `http://192.168.4.1/status` | 纯 JSON（可接脚本记录数据） |
| `http://192.168.4.1:81/stream` | 裸 MJPEG 流（VLC 打开"网络串流"也能看） |

## 实现要点（改了什么）

- `main/camera_stream.c`（新增）：WiFi soft-AP + 两个 esp_http_server 实例。流在 81 端口独立实例——流式 handler 会一直占用其服务器任务，独立开端口保证页面/状态永远响应。
- `main/camera_vision.c`：vision 任务在解码前把未解码的 MJPEG 原码流快照进独立 PSRAM 缓冲（try-lock，不拖慢视觉管线）；新增 `camera_vision_get_jpeg()`。
