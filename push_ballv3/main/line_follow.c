                                          ...
                                          int right_count)
{
    ...
    if (controller->lost_ms >= LINE_LOST_STOP_MS) {
        ...
        controller->stop_reason = LINE_STOP_REASON_LOST_TIMEOUT;
        reset_search(controller);
    ...
    if (old_state != LINE_FOLLOW_LOST_SEARCH ||
        ...
        if (controller->search_leg + 1U >= LINE_SEARCH_MAX_LEGS) {
            controller->state = LINE_FOLLOW_STOPPED;
            controller->stop_reason = LINE_STOP_REASON_SEARCH_EXHAUSTED;
            reset_search(controller);
        ...
        if (controller->search_progress_counts -
            ...
            if (controller->search_stall_ms >= LINE_SEARCH_STALL_MS) {
                controller->state = LINE_FOLLOW_STOPPED;
                controller->stop_reason = LINE_STOP_REASON_SEARCH_STALL;
                reset_search(controller);
                return stopped_result(true);
            }
        }
...
void line_follow_init(line_follow_controller_t *controller)
{
    ...
    *controller = (line_follow_controller_t) {
        ...
        .stop_reason = LINE_STOP_REASON_NONE,
    };
}
                                        ...
                                        int right_count)
{
    ...
    if (black_mask == IR_ALL_BLACK_MASK) {
        ...
        if (controller->all_black_ms >= LINE_ALL_BLACK_STOP_MS) {
            ...
            controller->state = LINE_FOLLOW_STOPPED;
            controller->stop_reason = LINE_STOP_REASON_FINISH;
            reset_search(controller);
...
{
    switch (reason) {
    case LINE_STOP_REASON_NONE:
        return "NONE";
    case LINE_STOP_REASON_FINISH:
        return "FINISH";
    case LINE_STOP_REASON_LOST_TIMEOUT:
        return "LOST_TIMEOUT";
    case LINE_STOP_REASON_SEARCH_EXHAUSTED:
        return "SEARCH_EXHAUSTED";
    case LINE_STOP_REASON_SEARCH_STALL:
        return "SEARCH_STALL";
    default:
        return "UNKNOWN";
    }
