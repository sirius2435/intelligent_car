...
void app_main(void)
{
    ...
    /*
     * BALL-ONLY TEST MODE:
     * Keep every original hardware/interface initialization above, but
     * skip the LINE phase.  The original ball_vision / ball_push modules
     * are used unchanged.  This makes power-on go directly to ball finding.
     * Ultrasonic, LCD, Wi-Fi stream, gimbal, encoder and drive interfaces
     * are still initialized exactly as in the original program.
     */
    ESP_LOGW(TAG, "BALL-ONLY TEST: skipping line-following; "
             "starting ball finding immediately");
    ...
    ball_push_controller_t push;
    run_phase_t phase = RUN_PHASE_BALL;
    ...
    /* Same hand-over used by the original finish-bar transition, but now
     * performed immediately after the first valid camera frame. */
    require_ok("camera mode push", camera_vision_set_mode(
                   CAMERA_VISION_MODE_PUSH));
    ball_push_init(&push);
    ESP_LOGW(TAG, "BALL-ONLY TEST: ball_push initialized; finding ball now");
        ...
        if (!locked && phase == RUN_PHASE_LINE) {
            ...
            if (avoid.active) {
                ...
                if (control.state == LINE_FOLLOW_STOPPED) {
                    if (controller.stop_reason == LINE_STOP_REASON_FINISH) {
                        /* Confirmed finish: hand the car over to the
                         * pocket-push task instead of locking forever. */
