...
static esp_err_t validate_pins(void)
{
    for (size_t i = 0; i < 4; ++i) {
        ...
        if (!GPIO_IS_VALID_GPIO(gpio)) {
            ESP_LOGE(TAG, "Channel %u GPIO is invalid or still -1; edit main/board_config.h",
                     (unsigned)i + 1U);
            return ESP_ERR_INVALID_ARG;
        }
        if (gpio == STATUS_LED_GPIO) {
            ESP_LOGE(TAG, "Channel %u duplicates status LED GPIO %d",
                     (unsigned)i + 1U, STATUS_LED_GPIO);
            return ESP_ERR_INVALID_ARG;
        }
        for (size_t j = i + 1; j < 4; ++j) {
            if (gpio == s_gpio_by_bit[j]) {
                ESP_LOGE(TAG, "GPIO %d is assigned to more than one infrared channel", gpio);
                return ESP_ERR_INVALID_ARG;
            }
        }
    }
...
{
    ESP_RETURN_ON_ERROR(validate_pins(), TAG, "unsafe infrared sensor pin configuration");
    ...
    ESP_RETURN_ON_ERROR(gpio_config(&input_config), TAG, "failed to configure sensor GPIOs");
    ...
    ESP_LOGI(TAG, "Four-channel infrared sensor initialized (left-to-right: 4,3,2,1)");
    return ESP_OK;
}
...
esp_err_t infrared_sensor_sample(infrared_sensor_state_t *state)
{
    ...
    state->changed = false;
    if (s_candidate_count >= IR_DEBOUNCE_SAMPLE_COUNT && s_stable_mask != s_candidate_mask) {
